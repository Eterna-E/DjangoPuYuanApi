...
def pressure_create_view(request):  # 8.上傳血壓測量結果
	...
	if request.method == "POST":
		try:
			...
			output = {"status": "0"}
			try:
				Pressure.objects.create(
					uid=uid, systolic=systolic, diastolic=diastolic, pulse=pulse, recorded_at=recorded_at)
			except:
				pass
		except:
			pass
		...
		try:
			...
			try:
				Weight.objects.create(
					uid=uid, weight=weight, body_fat=body_fat, bmi=bmi, recorded_at=recorded_at)
			except:
				pass
		except:
			pass
...
def sugar_create_view(request):  # 10.上傳血糖測量結果
	...
	if request.method == "POST":
		try:
			...
			output = {"status": "0"}
			try:
				Sugar.objects.create(uid=uid, exercise=exercise, drug=drug,
									 sugar=sugar, timeperiod=timeperiod, recorded_at=recorded_at)
			except:
				pass
		except:
			pass
...
def care(request):  # 28.送出關懷諮詢!+27.獲取關懷諮詢!
	...
	uid = authuser
	print('qweqweqrewrwertwerwerttwe')
	output = {"status": "1"}
	if request.method == 'POST':
		print(request.body)
		data = str(request.body).replace('b', '', 1).replace("\\r\\n", '').replace('\'', '')
	...
	if request.method == 'GET':
		print(12345667899)
		print(uid)
		usercares = UserCare.objects.filter(uid=uid)
		cares = []
		for usercare in usercares:
			r = {
				...
				# "updated_at": '1999-01-01 12:12:12'
			}
			if r != None:
				cares.append(r)
